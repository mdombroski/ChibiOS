...
/* Suppress warning about changing stack pointer */
#pragma diag_suppress=Ta020

/*
 * IAR MSP430 Calling Convention:
...
#else
  /* __get_SP_register() uses one extra mov instruction */
  otp->p_ctx.sp = (struct intctx __data16*) __get_SP_register();
#endif
...
void port_halt(void) {
  ...
  port_disable();
  while (TRUE) {
    /* Enter LPM4 (CPU off, clocks disabled) */
