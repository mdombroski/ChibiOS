 ...
 *
 * @param[out] sp       pointer to a @p chSemaphore structure
 ...
 *
 * @param[in] sp        pointer to a @p chSemaphore structure
...
void chSemSignalI(chSemaphore *sp) {
  ...
  if (++sp->s_cnt <= 0) {
    /* Note, it is done this way in order to allow a tail call on
             chSchReadyI().*/
    chThread *tp = fifo_remove(&sp->s_queue);
 ...
 *
 * @param[in] sps       pointer to a @p chSemaphore structure to be signaled
 * @param[in] spw       pointer to a @p chSemaphore structure to be wait on
 * @return              A message specifying how the invoking thread has been
 *                      released from the semaphore.
 * @retval RDY_OK       if the thread has not stopped on the semaphore or the
 *                      semaphore has been signaled.
